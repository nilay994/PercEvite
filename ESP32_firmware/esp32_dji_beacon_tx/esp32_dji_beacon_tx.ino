...
int wifi_send_beacon(uint64_t mac, const char *ssid, uint8_t channel)
{
    ...
    // unsure about last argument
    return esp_wifi_80211_tx(WIFI_IF_AP, wifi_beacon, sizeof(wifi_beacon), false);
}
...
int wifi_send_droneid(uint64_t mac, const char *ssid, uint8_t channel, void *packet, uint8_t length)
{
    ...
    Serial.println("**************************\n");
    for(int i = 0; i < (sizeof(wifi_beacon) + length); i++) {
      
      char tmpstr[4] = {0};
      sprintf(tmpstr, "0x%02x, ", wifi_droneid[i]);
      Serial.print(tmpstr);
    }
    Serial.println("**************************\n");
    ...
    return esp_wifi_80211_tx(WIFI_IF_AP, wifi_droneid, sizeof(wifi_beacon) + length, false);
}
